...
test('AI Search SP Filter Toggle', async ({ page }, testInfo) => {
  ...
  await page.click('.scm-ai-search-toggle-close')
  await page.waitForTimeout(500)
  await page.waitForSelector('[id="searchFilterToggle1"][aria-hidden="true"]', { state: 'attached' })
  await rootScreenshot(root, testInfo)
})

test('AI Search MD Filter Toggle', async ({ page }, testInfo) => {
...
test('AI Search LG Filter Toggle', async ({ page }, testInfo) => {
