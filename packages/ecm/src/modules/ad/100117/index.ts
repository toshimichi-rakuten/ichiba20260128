...
const dummyData = {
  ...
  '#TEXT3L2#': '',
  '#SHOPURL1#': '#',
  '#SHOP1#': 'ショップ名６７８９０ショップ名',
  '#TEXTURL5#': '#',
  '#TEXT5L1#': 'クーポンを獲得する',
  '#TEXT6L1#': '01/01(日)00:00まで',
}
...
export const staticSP1columnPC2columnHtml = ['店内全品', '対象アイテム限定'].reduce(
  (html, replacement) => html.replace('店内全品 / 対象アイテム限定', replacement),
  addGrid(soloWithDummySP1columnPC2columnHtml, SP1columnPC2column)
)
export const staticSP1columnPC3columnHtml = ['店内全品', '対象アイテム限定', '店内全品'].reduce(
  (html, replacement) => html.replace('店内全品 / 対象アイテム限定', replacement),
  addGrid(soloWithDummySP1columnPC3columnHtml, SP1columnPC3column)
)
export const staticSP1columnPC4columnHtml = ['店内全品', '対象アイテム限定', '店内全品', '対象アイテム限定'].reduce(
  (html, replacement) => html.replace('店内全品 / 対象アイテム限定', replacement),
  addGrid(soloWithDummySP1columnPC4columnHtml, SP1columnPC4column)
)
